...
const SHOPS = [
  ...
  },
  {
    ...
    description:
      "An artisan riverside café with organic coffee, healthy bites and a stunning outdoor bamboo terrace alongside the Beira Lake.",
    rating: 4.8,
    reviewCount: 526,
  ...
  },
  {
    ...
    description:
      "International café chain with a strong Sri Lankan twist — try their iced Ceylon tea latte and signature cold brew blends.",
    rating: 4.3,
    reviewCount: 189,
  ...
  },
  {
    ...
    tag: "Certified Gems",
    image: "https://images.unsplash.com/photo-1573408301185-9519f94815b3?w=600&q=80",
    openHours: "9 AM – 6 PM",
  ...
  },
  {
    ...
    description:
      "Gem capital of Sri Lanka! Visit a real gem mine, watch cutting & polishing, and buy direct from the source at unbeatable prices.",
    rating: 4.8,
    reviewCount: 512,
  ...
  },
  {
    ...
    location: "Colombo 3, Western Province",
    description:
      "Iconic design boutique famous for vibrant handwoven cotton textiles, unique clothing, home décor and an in-house garden café.",
    rating: 4.8,
    reviewCount: 892,
  ...
  },
  {
    ...
    description:
      "Specialising in authentic Kandyan sarees, traditional silk fabric and hand-embroidered ceremonial wear. Perfect for weddings & events.",
    rating: 4.6,
    reviewCount: 321,
  ...
  /* Souvenirs */
  {
    ...
    location: "Colombo 7, Western Province",
    description:
      "Iconic lifestyle store curated by renowned designer Shanth Fernando. Unique home décor, art prints, ceramics and exclusive Sri Lanka gifts.",
    rating: 4.8,
    reviewCount: 567,
...
export default function Shopping() {
  ...
  const visibleShops = useMemo(() => {
    return shops.filter((shop) => {
      ...
      const matchesArea = activeArea === AREA_ALL || getShopArea(shop.location) === activeArea;
      const q = searchTerm.toLowerCase().trim();
      const matchesSearch =
        !q ||
        shop.name.toLowerCase().includes(q) ||
        shop.category.toLowerCase().includes(q) ||
        getShopArea(shop.location).toLowerCase().includes(q) ||
        shop.location.toLowerCase().includes(q) ||
        shop.description.toLowerCase().includes(q) ||
        shop.tag.toLowerCase().includes(q);
      return matchesCat && matchesArea && matchesSearch;
    });
  }, [activeArea, activeCategory, searchTerm, shops]);
  ...
  const storeLabel = visibleShops.length === 1 ? copy.filters.storeSingular : copy.filters.storePlural;
  const emptyTitle = loading
    ? "Loading stores..."
    : error
      ? "Unable to load stores"
      : copy.sections.noStoresFound;
  const emptyBody = loading
    ? "Please wait while we load the latest store catalogue."
    : error || copy.sections.noStoresBody;
  ...
  return (
    <div className="sp">
          ...
          <div className="sp-tabs-scroll">
            {categories.map((cat) => (
              <button
                key={cat}
                type="button"
                className={`sp-tab${activeCategory === cat ? " sp-tab--on" : ""}`}
                onClick={() => setActiveCategory(cat)}
              >
                <span className="sp-tab-icon">{CATEGORY_ICONS[cat]}</span>
                {getShoppingCategoryLabel(language, cat)}
              </button>
            ))}
          </div>
          <div className="sp-tabs-count">
            <span>{visibleShops.length} {storeLabel}</span>
          </div>
        </div>
      </div>
          ...
          <div className="sp-section-head">
            <div>
              <p className="sp-kicker">
                {activeCategory === "All" ? copy.sections.allCategories : getShoppingCategoryLabel(language, activeCategory)}
              </p>
              <h2 className="sp-section-title">
                {searchTerm
                  ? `${copy.sections.resultsFor} "${searchTerm}"`
                  : activeCategory === "All"
                  ? `${copy.sections.featuredStoresIn} ${locationLabel}`
                  : `${getShoppingCategoryLabel(language, activeCategory)} ${copy.sections.in} ${locationLabel}`}
              </h2>
              <p className="sp-results-note sp-results-note--inline">
                {copy.sections.browseIntro}
              </p>
            </div>
          </div>
            ...
            <div className="sp-grid">
              {visibleShops.map((shop) => (
                <article
                  key={shop.id}
                  className="sp-card"
                  onClick={() => navigate(`/shopping/${shop.id}`)}
                  role="link"
                  tabIndex={0}
                  onKeyDown={(event) => {
                    if (event.key === "Enter" || event.key === " ") {
                      event.preventDefault();
                      navigate(`/shopping/${shop.id}`);
                    }
                  }}
                >
                  <div className="sp-card-img">
                    ...
                    <span className="sp-card-badge sp-card-badge--cat">
                      {CATEGORY_ICONS[shop.category]} {getShoppingCategoryLabel(language, shop.category)}
                    </span>
                  </div>
                  <div className="sp-card-body">
                    <h3 className="sp-card-title">{shop.name}</h3>
                    ...
                    <p className="sp-card-desc">{shop.description}</p>
                    <div className="sp-card-foot">
                      <div className="sp-stars">
                        {getRatingStars(shop.rating)}
                        <span>{getRatingText(shop.rating, shop.reviewCount, language)}</span>
                      </div>
        ...
        <div className="sp-container">
          <div className="sp-info-grid">
            <div className="sp-info-card">
              ...
              <h3>{copy.info.gemsTitle}</h3>
              <p>{copy.info.gemsBody}</p>
            </div>
            <div className="sp-info-card">
              <span className="sp-info-icon">🍵</span>
              <h3>{copy.info.teaTitle}</h3>
              <p>{copy.info.teaBody}</p>
            </div>
            <div className="sp-info-card">
              <span className="sp-info-icon">🧖</span>
              <h3>{copy.info.ayurvedaTitle}</h3>
              <p>{copy.info.ayurvedaBody}</p>
            </div>
            <div className="sp-info-card">
              <span className="sp-info-icon">👗</span>
              <h3>{copy.info.handloomTitle}</h3>
              <p>{copy.info.handloomBody}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <footer className="sp-footer">
        <Link to="/" className="sp-nav-brand">Ceylon Explore</Link>
        <p>{copy.footer}</p>
      </footer>
    </div>
  );
